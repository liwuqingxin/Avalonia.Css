...
namespace Nlnet.Avalonia.Css
{
    internal static class SelectorGrammar
    {
        ...
        internal static Selector? ToSelector(IEnumerable<SelectorGrammar.ISyntax> syntaxList)
        {
            return syntaxList.Aggregate<SelectorGrammar.ISyntax, Selector?>(null, (current, syntax) => syntax.ToSelector(current));
        }
        ...
        public class AttachedPropertySyntax : ISyntax, ITypeSyntax
        {
            ...
            public Selector? ToSelector(Selector? previous)
            {
                if (CssTypeProviderManager.Instance.TryGetType(TypeName, out var type))
                {
                    ...
                    var parserMethod = declareType.GetMethod("Parse", BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic);
                    if (parserMethod == null)
                    {
                        return previous;
                    }
                    return previous.PropertyEquals(avaloniaProperty, parserMethod.Invoke(declareType, new object?[] { Value }));
                }
        ...
        public class PropertySyntax : ISyntax
        {
            ...
            public Selector? ToSelector(Selector? previous)
            {
                ...
                {
                    var field            = previous.TargetType.GetField($"{Property}Property", BindingFlags.Public | BindingFlags.Static | BindingFlags.FlattenHierarchy);
                    var avaloniaProperty = field?.GetValue(previous.TargetType) as AvaloniaProperty;
                    if (avaloniaProperty == null)
                    {
                        return previous;
                    }
        ...
        public class NotSyntax : ISyntax
        {
            ...
            public Selector? ToSelector(Selector? previous)
            {
                var selector = SelectorGrammar.ToSelector(Argument);
                if (selector == null)
                {
                    return previous;
                }
